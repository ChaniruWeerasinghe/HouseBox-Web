...
// Insert before the Featured Items section
const target = '<!-- OUR FEATURED ITEMS (PARALLAX) SECTION -->';
if (content.includes(target)) {
    ...
    if (content.includes(insertBefore)) {
        content = content.replace(insertBefore, sectionHTML + '\n' + insertBefore);
    } else {
        content = content.replace(insertBefore2, sectionHTML + '\n' + insertBefore2);
    }
    fs.writeFileSync('index.html', content);
    console.log('Accordion injected successfully');
    console.log('accordion-item count:', (content.match(/accordion-item/g)||[]).length);
} else {
    console.error('Could not find Featured Items target');
}
